...
class User(GeneralMethods):
    ...
    def add_ptf_id(self, ptf_id: Union[str, ObjectId]):
        ...
        if self._isAuthentificated or self._verify_user():
            self._users_db.users.find_one_and_update(
                {"_id": self._id}, {"$push": {"ptf_ids": ptf_id}}
            )
        else:
    ...
    def generate_token(self=None, duration: int = 20) -> dict:
        ...
        result = {"value": token, "expiration": expiration}
        if self:
            self._users_db.tokens.find_one_and_update(
                {"_id": self._id}, {"$set": {"token": result}}, upsert=True
            )
        return result
