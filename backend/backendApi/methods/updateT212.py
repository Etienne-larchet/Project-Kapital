...
from classes.securities import Order, Stock, Fee
...
def init_client(client_id, ptf_index: int = 0):
    ...
    ptf = Portfolio(mongo_client, user.ptf_ids[ptf_index] if user.ptf_ids else None) # ptf[0] is always the 'basket ptf'
    if not user.ptf_ids:
...
def init_format(orders: list, t212: Trading212, base_currency: str):
    instruments = t212.get_instruments({'t212_id': list({order['t212_id'] for order in orders})})
    instruments_dict = {instrument['t212_id']: instrument for instrument in instruments}
    currencies = {instrument['currencyCode'] for instrument in instruments if instrument['currencyCode'] != base_currency}
    oldest_date = orders[-1]['dateModified'][:10]
    newest_date = orders[0]['dateModified'][:10]
    rates = ExchangeRate.convert(currencies,base_currency,date_from=oldest_date, date_to=newest_date)
    return instruments_dict, rates

@timing()
def format_orders(orders: list, ptf: Portfolio, rates: pd.DataFrame, instruments_dict: dict, base_currency: str):
    existing_stocks = {}
    for order in orders:
        ...
        if order['filledQuantity'] != None:
            ...
            if base_currency != currency:
                try:
                    ...
                    logger.warning(f"{order['dateModified'][:10]} not found in rates dataframes. Rate switched to {rates.index[-1]}")
                    rate = rates.iloc[-1][currency].item()

            else:
                rate = 1
            quantity = (order['filledValue']+ sum([fee.value for fee in fees]))*rate / order['fillPrice']
